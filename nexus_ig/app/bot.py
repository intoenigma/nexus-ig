...
from ..features import CommandHandler
    ...
    def sync_members(self, thread):
        ...
        for user_id, username in joined:
            ...
            if known_group and welcome_enabled and not getattr(self.config, "maintenance_mode", False):
                # 1. Share native Instagram profile card attachment (shows avatar & profile button)
                if self.cl and user_id:
                    try:
                        self.cl.direct_profile_share(user_id=str(user_id), thread_ids=[thread.pk])
                    except Exception as exc:
                        console.warning(f"Could not share profile card for @{username} on join: {exc}")

                # 2. Send short & clean welcome message
                short_welcome = (
                    f"👋 Welcome @{username} to {grp_title}! ✨\n"
                    f"📜 Rules: Respect members | No spam/scam links"
                )
                self.cl.direct_send(short_welcome, thread_ids=[thread.pk])
